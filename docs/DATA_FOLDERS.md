...
The `obj2brz` application uses a `data/` folder for storing imports, exports, logs, and user cache.
...
└── user_cache/        # User-specific cache data (can be flushed)
    └── logs/          # Application logs with timestamps
        └── obj2brz_YYYY-MM-DD_HH-MM-SS.log
```
...
```powershell
.\dist\obj2brz-windows-x86_64.exe
```
